...
##the permit number, then copy the permit folder & contents into that folder.
def FolderCopy():
    LogMessage(" Start the FolderCopy module.")        
    ...
    for folder in dirs:
        try:
            ...
            LogMessage ("Made Directory")
            LogMessage (" Try to find files in folder")
            
            for files in permitdir:
                name = files
                LogMessage(" " + name)
                LogMessage("Found this file inside new folder!")

                shutil.copy(NewPermitFolder + "/" + PermitID + "/" + files, permitFolder)
                LogMessage(" Copied files from new folder to the google temp permit folder!")
            
        except:
            LogMessage(" This didnt copy files inside folder!")
##        return
    return
